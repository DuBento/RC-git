...
bool_t resp_valCode(UDPConnection_t *udpConnec, UDPConnection_t *res, char *buffer, userInfo_t *userInfo) {
        char uid[BUFFER_SIZE] = { 0 }, vc[BUFFER_SIZE] = { 0 }, fname[BUFFER_SIZE] = { 0 }, fop;
        sscanf(buffer + strlen(REQ_VLC), " %s %s %c %s", uid, vc, &fop, fname);
...
bool_t resp_registerUser(char *status, userInfo_t *userInfo) {
        ...
        if (userInfo->uid)  free(userInfo->uid);
	if (userInfo->pass) free(userInfo->pass);
        userInfo->uid = NULL;
        userInfo->pass = NULL;
